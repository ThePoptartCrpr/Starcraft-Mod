	...
	public void onPlayerTick(PlayerTickEvent event) {
		if(event.phase == TickEvent.Phase.START && !event.player.worldObj.isRemote) {
			if(event.player.getHeldItemMainhand() != null) {
				if(BlockMovingLightSource.isLightEmittingItem(event.player.getHeldItemMainhand().getItem())) {
					...
					if(event.player.worldObj.getBlockState(blockLocation).getBlock() == Blocks.AIR) {
						event.player.worldObj.setBlockState(blockLocation, ModBlocks.BLOCK_MOVING_LIGHT_SOURCE.getDefaultState());
					} else if(event.player.worldObj
							.getBlockState(blockLocation.add(event.player.getLookVec().xCoord, event.player.getLookVec().yCoord, event.player.getLookVec().zCoord))
							.getBlock() == Blocks.AIR) {
						event.player.worldObj.setBlockState(
								blockLocation.add(event.player.getLookVec().xCoord, event.player.getLookVec().yCoord, event.player.getLookVec().zCoord),
