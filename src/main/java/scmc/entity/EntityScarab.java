...
public class EntityScarab extends EntityMob
{
	...
	public void onLivingUpdate()
	{
		if (!worldObj.isRemote)
		{
			for (int i = 0; i < 2; i++)
			{
				worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, posX + rand.nextDouble() * (double) width, posY + (rand.nextDouble() * (double) height),
						posZ + rand.nextDouble() * (double) width, 0, 0, 0);
			}
		}
