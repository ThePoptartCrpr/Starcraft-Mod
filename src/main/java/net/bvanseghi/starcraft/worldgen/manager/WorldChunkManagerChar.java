...
public class WorldChunkManagerChar extends WorldChunkManager {
	...
	public WorldChunkManagerChar(long par1, WorldType worldType) {
		this();
		GenLayer[] agenlayer = GenLayerChar.initGens(par1, worldType);
		agenlayer = getModdedBiomeGenerators(worldType, par1, agenlayer);
		this.genBiomes = agenlayer[0];
		this.biomeIndexLayer = agenlayer[1];
	}

	public WorldChunkManagerChar(World world) {
	...
	public ChunkPos ChunkPosfindBiomePosition(int par1, int par2, int par3, List list, Random rand) {
		IntCache.resetIntCache();
		int l = par1 - par3 >> 2;
		int i1 = par2 - par3 >> 2;
		int j1 = par1 + par3 >> 2;
		...
		for (int k2 = 0; k2 < l1 * i2; ++k2) {
			...
			if (list.contains(biome) && (chunk == null || rand.nextInt(j2 + 1) == 0)) {
				chunk = new ChunkPos(l2, i3);
				++j2;
			}
		}
