...
public class EntityLarva extends EntityZergPassive {
	...
	protected void applyEntityAttributes() {
		...
		getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(StarcraftConfig.LARVA_HEALTH);
	...
	public void moveEntity(double x, double y, double z) {
		if(noClip) {
			...
			if(canTriggerWalking() && !flag && !isRiding()) {
				...
				if(block == ModBlocks.ZERG_CREEP || block == Blocks.AIR || block == ModBlocks.KERATIN_CHUNK || block == ModMetaBlocks.ZERG_CARAPACE
						|| block == ModMetaBlocks.ZERG_FLESH) {
