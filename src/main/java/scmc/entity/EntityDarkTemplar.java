...
public class EntityDarkTemplar extends EntityProtossMob
{
	 ...
	 * @param lootingLvl
	 *            level of Looting enchant used to deliver killing blow of
	 *            entity
	 */
	@Override
	...
	public void onUpdate()
	{
		for (int i = 0; i < 2; ++i)
		{
			worldObj.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, posX + (rand.nextDouble() - 0.5D) * (double) width, posY + rand.nextDouble() * (double) height,
					posZ + (rand.nextDouble() - 0.5D) * (double) width, 0.0D, 0.0D, 0.0D, null);
		}
		super.onUpdate();
	}
