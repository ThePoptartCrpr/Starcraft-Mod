...
public class ModItemLayered extends ItemBlock	 {
	...
	public EnumActionResult onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
        if (stack.stackSize != 0 && playerIn.canPlayerEdit(pos, facing, stack))
        {
            ...
            if (block == this.block)
            {
                int i = ((Integer)iblockstate.getValue(ModBlockLayered.LAYERS)).intValue();
	...
	public boolean canPlaceBlockOnSide(World world, BlockPos pos, EnumFacing side, EntityPlayer player, ItemStack stack)
    {
        ...
        return (!(state.getBlock() instanceof ModBlockLayered) || ((Integer)state.getValue(ModBlockLayered.LAYERS)) > 7) ? super.canPlaceBlockOnSide(world, pos, side, player, stack) : true;
    }
}
