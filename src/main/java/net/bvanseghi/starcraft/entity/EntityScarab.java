...
public class EntityScarab extends EntityMob {
	private static final DataParameter<Integer> STATE = EntityDataManager.<Integer>createKey(EntityScarab.class,
			DataSerializers.VARINT);
	private static final DataParameter<Boolean> POWERED = EntityDataManager.<Boolean>createKey(EntityScarab.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> IGNITED = EntityDataManager.<Boolean>createKey(EntityScarab.class,
			DataSerializers.BOOLEAN);
	/**
	...
	public void onUpdate() {
		...
		if(!this.worldObj.isRemote && this.ticksExisted%40 == 0) {
			this.worldObj.spawnParticle(EnumParticleTypes.SMOKE_NORMAL,
					...
					this.posZ + (this.rand.nextDouble() - 0.5D) * (double) this.width, 0.0D, 0.0D, 0.0D, null);
		}
		super.onUpdate();
	}
