...
public class BiomeGenShakurasCity extends BiomesSC {

	public BiomeGenShakurasCity(BiomeProperties id) {
		super(id);

		setRegistryName("city_shakuras");

		topBlock = ModMetaBlocks.PROTOSS_METAL_T1.getStateFromMeta(1);
	...
	// Craft's Thoughts: So blockArray is pretty much the same as the
	// ChunkPrimer in that it holds the blocks in the chunk.
	// The primary difference is that the ChinkPrimer allows you to refer to the
	// array by x, y, z coords rather than having to do the conversion yourself.
	// I don't fully understand what this is doing, the variables are all
	// generic work variable names. But to convert it, you simply need to
	// seperate out the
	// X Y and Z coords, and use primer.setBlockState. And it seems x and y are
	// given, so I presume this is supposed to give the blocks in a single
	// column?
